...
// string data
SwatCalendar.week_names = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
...
SwatCalendar.prototype.buildControls = function() {
  ...
  for (var i = start_date.getFullYear(); i <= end_date.getFullYear(); i++) {
    if (i == yyyy) {
      ...
      year_array = year_array + '<option value="' + i + '">' + i + '</option>';
    }
  }
...
SwatCalendar.prototype.draw = function() {
  ...
  var calendar_start = this_year == start_year && this_month == start_month + 1;
  ...
  var prev_end_day =
    SwatCalendar.isLeapYear(yyyy) && prev_month == 2 ? 29 : dom[prev_month - 1];
  ...
  if (!this.open) {
    ...
    if (!this.positioned) {
      this.overlay.cfg.setProperty('context', [this.toggle_button, 'tl', 'bl']);
