...
class SwatTableViewInputRow {
   ...
   * @param {string} row_string the XML string to use when inserting a new row.
   *                            The XML string can contain '%s' placeholders.
   *                            Before a new row is added, the placeholders are
   *                            replaced with the new row index.
   */
   ...
   * @var boolean
   */
  static is_webkit = /AppleWebKit|Konqueror|KHTML/gi.test(navigator.userAgent);
   ...
   * @var Object
   */
  static parser = SwatTableViewInputRow_getXMLParser();
      ...
      if (child_node.nodeType == 1 && child_node.nodeName == 'td') {
        ...
        for (var j = 0; j < source_attributes.length; j++) {
          if (source_attributes[j].name == 'class') {
            ...
            dest_td.setAttribute(
              source_attributes[j].name,
